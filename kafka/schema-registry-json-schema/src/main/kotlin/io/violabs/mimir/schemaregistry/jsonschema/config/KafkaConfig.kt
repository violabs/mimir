...
class KafkaConfig {
    ...
    @Bean
    fun userEventV2KafkaTemplate(): KafkaTemplate<String, UserEventV2> {
        val configProps = mapOf<String, Any>(
            ProducerConfig.BOOTSTRAP_SERVERS_CONFIG to "localhost:9092",
            ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG to StringSerializer::class.java,
            ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG to CustomJsonSchemaSerializer::class.java,
    ...
    // Consumer factory for UserEventV2
    @Bean
    fun userEventV2ConsumerFactory(): ConsumerFactory<String, UserEventV2> {
        val configProps = mapOf<String, Any>(
            ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG to "localhost:9092",
            ConsumerConfig.GROUP_ID_CONFIG to "user-event-v2-consumer-group",
            ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG to StringDeserializer::class.java,
            ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG to CustomJsonSchemaDeserializer::class.java,
            AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG to "http://localhost:8090",
            KafkaJsonSchemaDeserializerConfig.JSON_VALUE_TYPE to UserEventV2::class.java.name,
            KafkaJsonSchemaDeserializerConfig.FAIL_INVALID_SCHEMA to true,
            ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG to false
        )
        return DefaultKafkaConsumerFactory(configProps)
    ...
    // Listener container factory for UserEventV2
    @Bean
    fun userEventV2KafkaListenerContainerFactory(): ConcurrentKafkaListenerContainerFactory<String, UserEventV2> {
        val factory = ConcurrentKafkaListenerContainerFactory<String, UserEventV2>()
        factory.consumerFactory = userEventV2ConsumerFactory()
        factory.containerProperties.ackMode = ContainerProperties.AckMode.MANUAL_IMMEDIATE
        return factory
    }
}